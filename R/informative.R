...
.informative = function(x, marker1, marker2 = NULL) {
  ...
  for (sub in nucs) {
    ...
    offs = sub$children
    link = sub$link
    is_uninf_leaf[link] = F # just in case link is an untyped child
    ...
    if (nkeep == 0 && link == mo && is_uninf_fou[fa])
      next
    if (nkeep == 1 && link == keep_offs && is_uninf_fou[fa] && is_uninf_fou[mo]) {
